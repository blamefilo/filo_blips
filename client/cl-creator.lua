...
do
    local stored = GetResourceKvpString('filo_blips')
    if stored then
        local decoded = json.decode(stored)
        if decoded then
            for k, v in pairs(decoded) do
                if type(k) == 'number' then
                    local id = v.id or tostring(k)
                    v.id = id
                    personalBlips[id] = v
                else
                    personalBlips[k] = v
            ...
            lib.registerContext({
                ...
                options = {
                    ...
                    },
                    {
                        ...
                        onSelect = function()
                            local validBlipImages = lib.callback.await('filo_blips:server:getValidBlipImages', nil)
                            if validBlipImages then
                                validBlipImages = msgpack.unpack(validBlipImages)
                            end
                            
                            local currentPage = 1
                            local spritesPerPage = 20
                            
                            local function loadBlipOptions()
                                local blipOptions = {}
                                local startIndex = (currentPage - 1) * spritesPerPage + 1
                                local endIndex = math.min(currentPage * spritesPerPage, 921)
                                
                                for i = startIndex, endIndex do
                                    if validBlipImages[i] then
                                        table.insert(blipOptions, {
                                            title = 'Blip: ' .. i,
                                            image = ('https://raw.githubusercontent.com/DemiAutomatic/fivem-blip-images/refs/heads/main/blips/%s.webp'):format(i),
                                            onSelect = function()
                                                blipData.sprite = i
                                                showManageOptions()
                                            end
                                ...
                                table.insert(blipOptions, {
                                    title = 'Next Page',
                                    description = 'Load next page of blip sprites',
                                    onSelect = function()
                                ...
                                lib.registerContext({
                                    id = 'edit_blip_sprite_selector',
                                    title = 'Change Sprite',
                                    options = blipOptions
                                })
                                lib.showContext('edit_blip_sprite_selector')
                            end
